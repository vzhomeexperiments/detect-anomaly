---
title: "Lecture 22 - Specialized R Packages"
output:
...
### Lecture 22 - Your project. Specialized R packages, AnomalyDetection, Wavelet Analysis

Hello everybody. Welcome back to one another bonus lecture. This one will be dedicated to specialized packages in R that are helping us to detect anomalies. 

These packages are using Statistical Methods which are helpful if you know there are seasonality in your data.

We will see how to use package 'AnomalyDetection' on the example on our data and also try 'Wavelet Analysis'.

This will be just a brief overview of functionality to get an idea...
...
Things to notice function is VERY long to run! Nevertheless it works. 

Intresting to notice that perhaps all data anomalies were detected...
...
Let's first make the object smaller:
```{r}
DF_M3 <- head(DF_M3_Tub_Res, 20000)
```
Here is our Wavelet transformation:

```{r}
res.wave <- morlet(y1 = DF_M3$AnalogVal, x1 = as.numeric(DF_M3$StartDate), p2 = 13, dj = 0.1, siglvl = 0.95)
```

This will be our plot:

```{r}
wavelet.plot(res.wave, useRaster=NA, add.coi=TRUE)
```
