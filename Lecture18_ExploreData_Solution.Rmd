...
```{r}
# we will create our graphs using for loop
library(magrittr)
# save our Event names into the vector
Events <- DF_TEMP %>% select(EventText) %>% unique() %$% EventText
# we will make dedicated for loop for that...
...
- Cutting Process, resistance is somewhat different on Machine #1
- Tubing Process, resistance requires some particular attention on Machines #2,3,4 
- Edging Process, phase angle on Machine #4 is more stable, on the rest we can observe some spikes up/down...
...
# Visualize the data...
DF_TEMP_All %>%
  ggplot(aes(x = StartDate, y = AnalogVal, col = EventText)) + geom_smooth()+facet_grid(~Name)
```
...
- It is very hard to grasp any tendency... we need to have some different approach on data wrangling before visualization