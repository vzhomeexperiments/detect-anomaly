...
DF_M1 <- DF_TEMP %>% 
  ...
  head(3000) %>% 
  t() %>%  # this brings us a matrix
  matrix(nrow = 150, ncol = 20) %>% # transforming that into matrix size 20x150
  t() # transpose to get desired structure
...
to_matrix <- function(x, filter_Event, filter_Machine, num_cols) {
  ...
  select(AnalogVal)
  # find number of rows
  nrows <- Step1 %>% nrow()/num_cols %>% round() 
  # extract relevant matrix
  Step2 <- Step1 %>% 
  t() %>%  # this brings us a matrix
  matrix(nrow = num_cols, ncol = nrows) %>% # transforming that into matrix size 20x150
  t() # transpose to get desired structure
  return(Step2)
}
```
...
```{r}
DF_M2 <- to_matrix(DF_TEMP, filter_Event = "Tubing Process, resistance Ohm",
                   filter_Machine = "Machine #2", num_cols = 150)
```
