---
title: "Lecture 14 - Theory and Task for Homework with Solution"
output:
...
obviously if you will use your input value only once you can do the same by:

```r
# using reactive value further in the code 
 filter(StartDateTime > input$DateStart)
```
...
Snippet of code below was taken directly from the `server.R` script. Use some `dummy` variables to verify your code inside `reactive` function that builds `DF_SUM`. Make sure that filter functions are properly working...

Hint: create another `dummy` variable give it the value `"Step 2 SubStep 6"`. Use it instead of `input$selInput`
...
############ Modify this code below to be able to run the code inside
 ...
 DF_SUM <- reactive({
  ...
  # run this code manually 
    DF_TEMP %>% 
      # filters for categories //replace input$... with a string...
      filter(EventText == Step) %>% 
      # group by 
      group_by(Name) %>% 
      # filters X date
      filter(StartDateTime > StartDate, StartDateTime < EndDate)
      
      # chunk can be now selected and executed with CTRL + ENTER
...
# run this code manually 
    DF_TEMP %>% 
      ...
      group_by(Name) %>% 
      # filters X date
      filter(StartDateTime > StartDate, StartDateTime < EndDate) %>% head()
```
