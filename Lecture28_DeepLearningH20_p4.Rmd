...
#### General Process to Train Models

This code contains a procedure to train Deep Learning models that will make Shiny App working.

This procedure will have to be repeated anytime when h2o source package is updated! Document will contain detailed procefure to select data for one process as well as the code to make a quicker model training.
...
#### Step by step model generation for one process

It is necessary to visualize the data from several machines to select the data which is more closer to the specifications. 'Domain' or 'Specialist' knowledge will need to be applied:
...
Then we will choose the data which will be used for modeling:
...
Data will need to be transformed to the 'image' shape:

```{r, eval=FALSE, include=TRUE}
# transform to 2 matrixes
source("to_m.R")

DF_M1 <- to_m(DF_TEMP_TPA, 150) %>% as.data.frame()

```

We can visualize this shape in 3D

```{r, eval=FALSE, include=TRUE}
# plotly
plot_ly(z = as.matrix(DF_M1), type = "surface")
