...
# Data manipulation and saving to the DF_Data reactive value
    DF_KM <- DF_TEMP %>% filter(EventText == "Step 1 SubStep 1") 
...
```{r}
# Some points above level 1000 are probably either anomalies or data artefacts
# It's anyway strange that these points are never present on unit '4'
```
...
```{r}
# probably just the scale is different...
```
...
```{r}
# Step 1 Substep 1, Step 1 Substep 3, Step 1 Substep 4 are definitely troubled with scaling. The rest is totally the same
# Unsupervised Machine Learning is known to be difficult and for that reason other methods exist such as hierarchical  clustering
# Probably in this case the data for Step 1 is completely randomly generated without any rule and for that scaling was able to 'catch' this phenomena.
# It would however be much advisable still to have the data scaled rather than not and make sure there are no outliers if possible
```
