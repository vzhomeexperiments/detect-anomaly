...
# function that perform scoring using Deep Learning model to each machine and return common dataframe
anomalyscore_machines <- function(x, Machines){
  ...
  for(i in 1:length(Machines, path_to_model)){
    ...
    DF_F <- anomalyscore_nn(DF_A, path_to_model)
...
anomalyscore_nn <- function(x, path_to_model){
